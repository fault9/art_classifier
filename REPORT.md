...
Images were resized and represented using `openai/clip-vit-base-patch32`, producing 512-dimensional image embeddings. Several classifiers were evaluated on the embeddings: Logistic Regression, SVM-RBF, and an MLP. The final selected model was the MLP classifier trained on CLIP embeddings. Evaluation used a fixed train/test split with 1,090 training images and 388 held-out test images. The training script also performs cross-validation to compare model choices.
